...
import { QuadrenniumGoalsTab } from "@/components/modals/masters/variables/goals/QuadrenniumGoalsTab";
...
const mockGoals = {
  data: [
    { id: "qg1", variableId: "v1", startYear: 2024, endYear: 2027, value: "200", createAt: "2024-01-01T00:00:00.000Z" },
    { id: "qg2", variableId: "v1", startYear: 2020, endYear: 2023, value: "150", createAt: "2023-01-01T00:00:00.000Z" },
  ],
  meta: { total: 2, page: 1, limit: 5, totalPages: 1 },
};

describe("QuadrenniumGoalsTab", () => {
  beforeEach(() => {
    jest.clearAllMocks();
  ...
  it("renders and fetches quadrennium goals on mount", async () => {
    render(<QuadrenniumGoalsTab variableId="v1" />);
    await waitFor(() => expect(mockGet).toHaveBeenCalled());
  });

  it("does not fetch when variableId is null", () => {
    render(<QuadrenniumGoalsTab variableId={null} />);
  ...
  it("renders empty state when no goals", async () => {
    ...
    render(<QuadrenniumGoalsTab variableId="v1" />);
    await waitFor(() => expect(screen.getByText("No hay metas por cuatrenio registradas para esta variable")).toBeInTheDocument());
  });
});
