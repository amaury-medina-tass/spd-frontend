...
export interface LocationModalProps {
    ...
    /** Optional code to display in the header */
    entityCode?: string
    /** Subtitle text for search mode */
    subtitle?: string
    /** Fetch associated locations for the entity */
    fetchLocations: (entityId: string) => Promise<any[]>
    /** Associate a location to the entity */
    associateLocation: (entityId: string, locationId: string) => Promise<void>
    /** Disassociate a location from the entity */
    disassociateLocation: (entityId: string, locationId: string) => Promise<void>
}
...
function getCommuneDisplayValue(communes: Commune[], value: string): string {
    const sel = communes.find(c => c.id === value);
    return sel ? `${sel.code} - ${sel.name}` : '';
}

export function LocationModal({
    isOpen, onClose, entityId, entityCode,
    subtitle = "Gestiona las ubicaciones asociadas",
    fetchLocations, associateLocation, disassociateLocation
}: Readonly<LocationModalProps>) {
    ...
    const { control, handleSubmit, formState: { errors, isSubmitting: isCreating }, reset, setValue, watch } = useForm<CreateLocationFormValues>({
        ...
        defaultValues: { address: "", communeId: "", latitude: "", longitude: "" }
    })
    ...
    useEffect(() => {
        if (createMode === "builder") {
            ...
            if (roadLabel && roadNumber) {
                ...
                if (additionalInfo) address += `, ${additionalInfo}`
            }
    ...
    const fetchAssociatedLocations = async () => {
        ...
        try {
            const data = await fetchLocations(entityId)
    ...
    }, [isOpen, reset, entityId])

    // Fetch locations when search changes
    useEffect(() => {
        if (mode === "search" && debouncedSearch.length >= 3) {
            setLoadingLocations(true)
            const params = new URLSearchParams({ search: debouncedSearch, limit: "20", offset: "0" })
    ...
    useEffect(() => {
        if (mode === "create" && communes.length === 0) {
            ...
            getCommunesSelect("limit=100")
                ...
                .catch(() => addToast({ title: "Error al cargar comunas", color: "danger" }))
    ...
    const handleAssociate = async (locationId: string) => {
        if (!entityId) return
        try {
            await associateLocation(entityId, locationId)
    ...
    const handleDisassociate = async (locationId: string) => {
        ...
        try {
            await disassociateLocation(entityId, locationId)
                            ...
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    <h2 className="text-base font-semibold text-default-900">
                                        ...
                                        {entityCode && <span className="ml-2 text-primary">({entityCode})</span>}
                                    </h2>
                                    <p className="text-xs text-default-400 font-normal">
                                        {mode === "search" ? subtitle : "Completa los datos de la ubicación"}
                                    </p>
                                </div>
                            </div>
                        </ModalHeader>
                                    ...
                                    <form id="create-location-form" onSubmit={handleSubmit(handleCreateSubmit)} className="space-y-5">
                                        ...
                                        <Controller
                                            ...
                                            render={({ field }) => (
                                                <Autocomplete
                                                    ...
                                                    inputValue={getCommuneDisplayValue(communes, field.value)}
                                                    onInputChange={() => {}}
                                        ...
                                        <div>
                                            <div className="flex items-center justify-between mb-2">
                                                <span className="text-sm text-default-600">Dirección</span>
                                                <button
                                                    type="button"
                                                    onClick={() => {
                                                        if (createMode === "builder") {
                                        ...
                                        {/* Coordinates */}
                                        <div>
                                            <span className="text-sm text-default-600 mb-2 block">Coordenadas (opcional)</span>
