...
import { associateActionPlanIndicatorLocation, associateIndicativePlanIndicatorLocation, getActionPlanIndicatorLocations, getIndicativePlanIndicatorLocations, disassociateActionPlanIndicatorLocation, disassociateIndicativePlanIndicatorLocation } from "@/services/masters/indicators.service"
import { z } from "zod"
...
export function IndicatorLocationModal({ isOpen, onClose, indicatorId, type }: IndicatorLocationModalProps) {
    ...
    const fetchAssociatedLocations = async () => {
        ...
        try {
            const data = type === "action"
                ? await getActionPlanIndicatorLocations(indicatorId)
                : await getIndicativePlanIndicatorLocations(indicatorId)
    ...
    const handleAssociate = async (locationId: string) => {
        ...
        try {
            if (type === "action") {
                await associateActionPlanIndicatorLocation(indicatorId, locationId)
            } else {
                await associateIndicativePlanIndicatorLocation(indicatorId, locationId)
            }
    ...
    const handleDisassociate = async (locationId: string) => {
        ...
        try {
            if (type === "action") {
                await disassociateActionPlanIndicatorLocation(indicatorId, locationId)
            } else {
                await disassociateIndicativePlanIndicatorLocation(indicatorId, locationId)
            }
                            ...
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <p className="text-xs text-default-400 font-normal">
                                        {mode === "search" ? "Gestiona las ubicaciones asociadas" : "Completa los datos de la ubicación"}
                                    </p>
                                </div>
                            </div>
                        </ModalHeader>
