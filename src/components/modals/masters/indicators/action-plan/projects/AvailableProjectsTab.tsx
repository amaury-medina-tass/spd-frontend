...
import { getActionPlanIndicatorProjects, associateActionPlanIndicatorProject } from "@/services/masters/indicators.service"
...
export function AvailableProjectsTab({ indicatorId }: Props) {
    ...
    const fetchProjects = useCallback(async () => {
        ...
        try {
            ...
            addToast({ title: "Error al cargar proyectos disponibles", description: error.message, color: "danger" })
        } finally {
            setLoading(false)
        }
    ...
    const handleAssociate = async (projectId: string) => {
        ...
        try {
            ...
            addToast({ title: "Proyecto asociado", color: "success" })
            fetchProjects()
        } catch (e: any) {
            ...
            case "actions":
                return (
                    <Button
                        ...
                        title="Asociar"
                    >
                        {actionLoading !== item.id && <Plus size={16} />}
                    </Button>
                )
            default:
                return null
            ...
            emptyContent={
                <div className="flex flex-col items-center justify-center py-12 text-default-400">
                    ...
                    <p className="text-sm font-medium">
                        {search ? "No se encontraron proyectos para la búsqueda" : "No hay proyectos disponibles"}
                    </p>
                </div>
            }
        />
