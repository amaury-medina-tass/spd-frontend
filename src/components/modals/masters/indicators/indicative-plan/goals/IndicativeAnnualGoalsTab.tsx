...
export function IndicativeAnnualGoalsTab({ indicatorId }: Props) {
    const [goals, setGoals] = useState<IndicativePlanIndicatorGoal[]>([])
    ...
    const [editingGoal, setEditingGoal] = useState<IndicativePlanIndicatorGoal | null>(null)
    const [goalToDelete, setGoalToDelete] = useState<IndicativePlanIndicatorGoal | null>(null)
    ...
    const fetchGoals = useCallback(async () => {
        ...
        try {
            ...
            const result = await get<PaginatedData<IndicativePlanIndicatorGoal>>(`${endpoints.masters.indicativePlanIndicatorGoals}?${params}`)
            setGoals(result.data)
            setMeta(result.meta)
        } catch (error) {
            console.error("Error fetching indicative plan indicator goals:", error)
        } finally {
            setLoading(false)
        }
    ...
    const handleEditGoal = (goal: IndicativePlanIndicatorGoal) => {
        setEditingGoal(goal)
    ...
    const handleDeleteGoal = (goal: IndicativePlanIndicatorGoal) => {
    ...
    const onSubmit = async (data: FormValues) => {
        ...
        try {
            if (editingGoal) {
                await patch(`${endpoints.masters.indicativePlanIndicatorGoals}/${editingGoal.id}`, {
                    value: data.value
                })
                addToast({ title: "Meta actualizada correctamente", color: "success" })
                setEditingGoal(null)
            } else {
                await post(endpoints.masters.indicativePlanIndicatorGoals, {
                    indicatorId,
                    year: data.year,
                    value: data.value,
                })
    ...
    const onConfirmDelete = async () => {
        ...
        setDeleting(true)
        try {
            await del(`${endpoints.masters.indicativePlanIndicatorGoals}/${goalToDelete.id}`)
    ...
    const renderCell = (goal: IndicativePlanIndicatorGoal, columnKey: React.Key) => {
        switch (columnKey) {
            case "year":
