...
export function IndicativeQuadrenniumGoalsTab({ indicatorId }: Props) {
    const [quadrenniumGoals, setQuadrenniumGoals] = useState<IndicativePlanIndicatorQuadrenniumGoal[]>([])

    // No pagination for this endpoint as it uses get without params in the by-indicator endpoint
    const [quadrenniumLoading, setQuadrenniumLoading] = useState(false)

    const [editingQuadrennium, setEditingQuadrennium] = useState<IndicativePlanIndicatorQuadrenniumGoal | null>(null)
    const [quadrenniumToDelete, setQuadrenniumToDelete] = useState<IndicativePlanIndicatorQuadrenniumGoal | null>(null)
    ...
    const fetchQuadrenniumGoals = useCallback(async () => {
        ...
        try {
            const result = await get<IndicativePlanIndicatorQuadrenniumGoal[]>(endpoints.masters.indicativePlanIndicatorQuadrenniumsByIndicator(indicatorId))
            // The endpoint returns data directly from the array in the response shown by the user
            setQuadrenniumGoals(Array.isArray(result) ? result : (result as any).data || [])
        } catch (error) {
            console.error("Error fetching indicative plan indicator quadrennium goals:", error)
        } finally {
    ...
    const handleEditQuadrennium = (goal: IndicativePlanIndicatorQuadrenniumGoal) => {
    ...
    const handleDeleteQuadrennium = (goal: IndicativePlanIndicatorQuadrenniumGoal) => {
    ...
    const onQuadrenniumSubmit = async (data: QuadrenniumFormValues) => {
        ...
        try {
            if (editingQuadrennium) {
                await patch(`${endpoints.masters.indicativePlanIndicatorQuadrenniums}/${editingQuadrennium.id}`, {
                    value: data.value
                })
                addToast({ title: "Meta cuatrenio actualizada correctamente", color: "success" })
                setEditingQuadrennium(null)
            } else {
                await post(endpoints.masters.indicativePlanIndicatorQuadrenniums, {
                    indicatorId,
    ...
    const onConfirmDelete = async () => {
        ...
        setDeleting(true)
        try {
            await del(`${endpoints.masters.indicativePlanIndicatorQuadrenniums}/${quadrenniumToDelete.id}`)
    ...
    const renderQuadrenniumCell = (goal: IndicativePlanIndicatorQuadrenniumGoal, columnKey: React.Key) => {
        switch (columnKey) {
            case "startYear":
    ...
    return (
        <div className="pt-4 space-y-6">
            ...
            <ResourceManager
                ...
                // No pagination meta for this simple list
                emptyContent={
